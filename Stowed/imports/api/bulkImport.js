...
const ALLOWED_STORAGE_UNIT_TYPES = new Set([
  "shelf",
  "cabinet",
  "rack",
  "drawer",
  ...
  if (Array.isArray(value)) {
    return value
      .map((assignment) => {
        ...
        const quantity =
          getOptionalInteger(
            getField(assignment, "quantity", "qty", "totalQuantity", "total_quantity"),
          ) || 0;
...
function mergeAssignmentsByLocation(assignments) {
  ...
  for (const { locationId, quantity } of assignments) {
    quantitiesByLocationId.set(
      locationId,
      (quantitiesByLocationId.get(locationId) ?? 0) + quantity,
    );
  }
  return Array.from(quantitiesByLocationId.entries()).map(([locationId, quantity]) => ({
    locationId,
    quantity,
  }));
}
...
async function findOrCreateStorageUnit({
  orgId,
  floorMapId,
  name,
  type = "other",
  ...
  if (existing) {
    ...
    if (width !== undefined || height !== undefined) {
      updates.shape = createDefaultShape(orgId, width ?? 2, height ?? 1);
    }
...
Meteor.methods({
  async "bulk.importLocations"(csvText) {
    ...
    for (const r of rows) {
      ...
      const floorName =
        getField(r, "floorMapName", "floorMap.name", "floor_map_name") || "Default Floor";
      const unitName =
        getField(r, "storageUnitName", "storageUnit.name", "storage_unit_name") || "Unit";
      const unitType =
        getField(r, "storageUnitType", "storageUnit.type", "storage_unit_type") || "other";
      const locationName =
        getField(r, "locationName", "storageLocation.name", "location_name") || "Location";
      const locationCode =
        getField(r, "locationCode", "storageLocation.code", "location_code") || "";
      const lastStocktakeAt =
        getOptionalDate(
          getField(r, "lastStocktakeAt", "storageLocation.lastStocktakeAt", "last_stocktake_at"),
        ) ?? now;
  ...
  async "bulk.importProducts"(csvText) {
    ...
    for (const r of rows) {
      ...
      if (assignmentsRaw) {
        for (const assignment of parseAssignmentEntries(assignmentsRaw)) {
          ...
          if (!loc) {
            ...
            if (!defaultSiteId) {
              defaultSiteId = await Sites.insertAsync({
                orgId,
                name: "Imported Site",
            ...
            if (!defaultFloorId) {
              defaultFloorId = await FloorMaps.insertAsync({
                orgId,
            ...
            if (!defaultUnitId) {
              ...
              defaultUnitId = await StorageUnits.insertAsync({
                orgId,
                floorMapId: defaultFloorId,
                name: "Imported Unit",
                type: "other",
                shape,
                offset: { x: 0, y: 0 },
      ...
      if (existingProduct) {
        ...
        if (!restockHandler)
          throw new Meteor.Error("server-error", "Products restock handler not available");
        if (totalQuantity > 0) {
  ...
  async "bulk.importCombined"(payload) {
    ...
    const fileName =
      typeof payload === "string" ? "Imported data" : payload.fileName || "Imported data";
    check(csvText, String);
    ...
    try {
      for (const r of rows) {
        ...
        const floorName = (
          getField(r, "floorMapName", "floorMap.name", "floor_map_name") || ""
        ).trim();
        const unitName = (
          getField(r, "storageUnitName", "storageUnit.name", "storage_unit_name") || ""
        ).trim();
        const unitType = (
          getField(r, "storageUnitType", "storageUnit.type", "storage_unit_type") || ""
        ).trim();
        const locationName = (
          getField(r, "locationName", "storageLocation.name", "location_name") || ""
        ).trim();
        const locationCode = (
          getField(r, "locationCode", "storageLocation.code", "location_code") || ""
        ).trim();
        ...
        const floorMapWidth = getOptionalNumber(
          getField(r, "floorMapWidth", "floorMap.width", "floor_map_width"),
        );
        const floorMapHeight = getOptionalNumber(
          getField(r, "floorMapHeight", "floorMap.height", "floor_map_height"),
        );
        const floorSize =
          floorMapWidth !== undefined || floorMapHeight !== undefined
            ? toImportedFloorSize({
                widthMeters: floorMapWidth ?? 10,
                heightMeters: floorMapHeight ?? 10,
              })
            : undefined;
        const unitOffsetX = getOptionalNumber(
          getField(
            r,
            "storageUnitOffsetX",
            "storageUnit.offset.x",
            "storage_unit_offset_x",
            "unitX",
            "x",
          ),
        );
        const unitOffsetY = getOptionalNumber(
          getField(
            r,
            "storageUnitOffsetY",
            "storageUnit.offset.y",
            "storage_unit_offset_y",
            "unitY",
            "y",
          ),
        );
        const unitOffset =
          unitOffsetX !== undefined || unitOffsetY !== undefined
            ? { x: unitOffsetX ?? 0, y: unitOffsetY ?? 0 }
            : undefined;
        const unitWidth = getOptionalNumber(
          getField(
            r,
            "storageUnitWidth",
            "storageUnit.width",
        ...
        const unitHeight = getOptionalNumber(
          getField(
            r,
            "storageUnitHeight",
            "storageUnit.height",
        ...
        if (floorName) {
          ...
          const floorResult = await findOrCreateFloorMap({
            orgId,
            siteId: parentSiteId,
            name: floorName,
            floorSize,
