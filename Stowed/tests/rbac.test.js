...
describe("Role-Based Access Control", function () {
  ...
  it("denies access for unknown permissions", function () {
    assert.strictEqual(hasClientPermission(ROLES.ADMIN, "route:/unknown"), false);
  });
  ...
  it("allows elevated roles to access standard routes", function () {
    assert.strictEqual(hasClientPermission(ROLES.ADMIN, "route:/inventory"), true);
    assert.strictEqual(hasClientPermission(ROLES.OWNER, "route:/inventory"), true);
  });

  it("enforces admin-only product permissions", function () {
    assert.strictEqual(hasClientPermission(ROLES.STANDARD, "products.create"), false);
    assert.strictEqual(hasClientPermission(ROLES.ADMIN, "products.create"), true);
    assert.strictEqual(hasClientPermission(ROLES.OWNER, "products.create"), true);
  });

  it("restricts stocktake settings to admins and owners", function () {
    ...
    assert.strictEqual(hasClientPermission(ROLES.OWNER, "route:/settings"), true);
  ...
  it("enforces standard-level restock permission", function () {
    ...
    assert.strictEqual(hasClientPermission(ROLES.OWNER, "products.restock"), true);
  });
