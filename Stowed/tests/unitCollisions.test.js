...
} from "../imports/ui/pages/floorMapComponents/canvas/editor/utils/UnitCollisions";
...
describe("Collision math calculations", function () {
  it("test 'isRangeIntersecting' where A strictly < B", function () {
    ...
    assert.ok(
      ...
      "the intervals " +
        ...
        " and " +
        intervalB +
        "were expected to get overalapping=" +
        expected +
        ", but returned overalapping=" +
        result,
    );
  });
  ...
  it("test 'isRectRectIntersecting' BV corners and edges", function () {
     ...
     */
    const editsRectB = edits.flatMap((x) =>
      edits.flatMap(
        (y) =>
          edits
            .map((bv) => ({ editX: x, editY: y, bvTest: bv }))
    ...
    assert.ok(
      editsRectB.length == 24,
      "Expected " +
        expectedEdits +
        " border test cases between two rectangles but found " +
        editsRectB.length,
    );
  ...
  it("test 'calcDistance' for positive and negative numbers", function () {
    ...
    const allTriples = magnitudes.flatMap((m) =>
      signs.flatMap((s1) =>
        signs.flatMap((s2) =>
          pythagTriples.map(({ a, b, c }) => ({
            a: a * m * s1,
            b: b * m * s2,
            x1: x,
            x2: x + a * m * s1,
    ...
    allTriples.forEach(({ x1: x1, x2: x2, y1: y1, y2: y2, c: c }) => {
      ...
      assert.ok(
        result === expected,
        "expected the distance between the points (" +
          x1 +
          ", " +
          y1 +
          ") and (" +
  ...
  it("test 'isCircleCircleIntersecting' BV outside borders", function () {
    ...
    assert.ok(
      result === expected,
      "the circles " +
        JSON.stringify(circ1) +
        " and " +
        JSON.stringify(circ2) +
        "were expected to get overalapping=" +
        expected +
        ", but returned overalapping=" +
        result,
